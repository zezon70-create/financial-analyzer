  ...
  function renderRows() {
    ...
    trial.forEach((r, idx) => {
      ...
      tr.innerHTML = `
        ...
        <td><input data-idx="${idx}" data-field="debit" type="number" value="${r.debit || 0}" class="financial-value" /></td>
        <td><input data-idx="${idx}" data-field="credit" type="number" value="${r.credit || 0}" class="financial-value" /></td>
  ...
  function highlightBalance(debit, credit) {
    ...
    if (Math.abs(debit - credit) < 0.01) {
      ...
      validationResult.innerHTML =
        `<div class="alert alert-success">ميزان المراجعة متوازن ✅. مجموع المدين: ${fmt(debit, currencySelect.value)}, مجموع الدائن: ${fmt(credit, currencySelect.value)}</div>`;
    } else {
      tbBody.querySelectorAll('tr').forEach(tr => tr.classList.add('unbalanced'));
      validationResult.innerHTML =
        `<div class="alert alert-danger">ميزان المراجعة غير متوازن ❌! مجموع المدين: ${fmt(debit, currencySelect.value)}, مجموع الدائن: ${fmt(credit, currencySelect.value)}</div>`;
    }
  }
