/* input.js — النسخة النهائية المحسّنة: قراءة CSV/XLSX, تحرير الجدول, حفظ/تحميل, dark-mode, i18n, currency formatting */
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fmtCurrency = (v, curr = (currencySelect ? currencySelect.value : 'EGP')) => {
    if (v == null || Number.isNaN(Number(v))) return '-';
    try {
      return new Intl.NumberFormat((langSelect && langSelect.value==='ar')?'ar-EG':'en-US',{
  ...
  function normalizeRows(rows) {
    ...
    return rows.map(r => {
      ...
      if ((debit===0 && credit===0) && value !== undefined){
        ...
        return v>=0 ? {account, code, debit:v, credit:0, type:r.type||''} : {account, code, debit:0, credit:Math.abs(v), type:r.type||''};
      }
  ...
  function renderRows() {
    ...
    trial.forEach((r, idx) => {
      ...
      tr.querySelector('.btn-del').addEventListener('click',()=>{ window.__trialData.splice(idx,1); saveToLocal(); renderRows(); });
    });
    attachRowEvents();
  ...
  saveBtn.addEventListener('click',()=>{ saveToLocal(); showMessage('تم حفظ الجلسة محلياً.'); });
  ...
  if(exportExcelBtn) exportExcelBtn.addEventListener('click',()=>{
    try{
      ...
      const ws=XLSX.utils.json_to_sheet(trial.map(r=>({الحساب:r.account, رمز:r.code, مدين:r.debit, دائن:r.credit, النوع:r.type})));
      XLSX.utils.book_append_sheet(wb,ws,'ميزان المراجعة');
      XLSX.writeFile(wb,'trial-balance.xlsx');
    }catch(e){ console.error(e); alert('فشل التصدير إلى Excel'); }
  });
  ...
  // Dark Mode / Currency / Language
  if(darkToggle){
    const stored = localStorage.getItem('darkMode')==='true';
    darkToggle.checked=stored;
    document.body.classList.toggle('dark-mode',stored);
    darkToggle.addEventListener('change',e=>{ document.body.classList.toggle('dark-mode',e.target.checked); localStorage.setItem('darkMode',e.target.checked?'true':'false'); });
  }
