...
document.addEventListener('DOMContentLoaded', () => {
            ...
            const statements = state.statements; // Use the freshly reset structure
             ...
             try {
                trialData.forEach(row => {
                    const value = (toNum(row.Debit)) - (toNum(row.Credit)); // *** USE toNum ***
    ...
    const renderBalanceSheet = () => {
        ...
        const totalLiabsAndEquity = totalLiabs + totalEquityValue + (state.financials.netProfit || 0); // Add Net Profit to Equity for balance
    ...
    const renderCashFlowStatement = () => {
        ...
        let html = `<table class="table table-sm report-table"><tbody>
            ...
            <tr class="table-light fw-bold"><td>${t_page('cfOperating')}</td><td class="text-end">${formatCurrency(f.ocf_estimated)}</td></tr>
            <tr><td>${t_page('cfInvesting')}</td><td class="text-end">${formatCurrency(f.icf_estimated)}</td></tr>
            <tr><td>${t_page('cfFinancing')}</td><td class="text-end">${formatCurrency(f.fcf_estimated)}</td></tr>
            </tbody></table><table class="table report-table"><tbody class="total-row"><tr><td>${t_page('netCashFlow')}</td><td class="text-end">${formatCurrency(f.netCashChange_estimated)}</td></tr></tbody></table>`;
    ...
    const renderEquityStatement = () => {
        ...
        // Calculate opening equity (Total Equity - Net Profit)
        const netProfit = state.financials.netProfit || 0;
        const closingEquity = state.financials.equity + netProfit; // Total Equity from TB is usually opening + profit
        const openingEquity = state.financials.equity; // Assuming TB equity is opening balance
    ...
    const init = () => {
        ...
        if (state.hasValidData) {
         ...
         }, 100); // Delay of 100ms to ensure main.js runs
