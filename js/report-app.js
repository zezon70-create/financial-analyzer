// js/advanced-app.js (Full Version + Benchmarks + setTimeout Delay)
...
window.pageTranslations = {
    ...
    en: { // ... (All English translations as before) ...
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // *** MODIFICATION: Wrap entire logic in a setTimeout to delay execution ***
    setTimeout(() => {
        ...
        // ==============================================
        // === FINANCIAL CALCULATIONS (Full version) ===
        // ==============================================
        ...
        const renderRatioCategory = (divId, categoryTitleKey, ratioKeys) => {
            console.log(`[DEBUG] Attempting to render ratio category: ${categoryTitleKey} into div: ${divId}`);
            const container = document.getElementById(divId);
            if (!container) { console.error(`[DEBUG] Element not found: ${divId}`); return; }
            if (!state.hasValidData) {
                console.warn(`[DEBUG] No valid data for ${categoryTitleKey}, showing warning.`);
                container.innerHTML = `<h5 class="mb-3">${t_page(categoryTitleKey)}</h5> <p class="text-muted">${t_page('noDataForRatios')}</p>`; return;
            }
            console.log(`[DEBUG] Rendering ${categoryTitleKey} with data...`);
            ...
            console.log(`[DEBUG] Finished rendering ${categoryTitleKey}.`);
        };
        ...
        const runAnalysis = (renderOnlyRatios = false) => {
            ...
            state.hasValidData = calculateFinancials() && calculateAllRatios();
        ...
        const populateIndustrySelect = () => {
            if (!UI.industrySelect) { console.warn("[DEBUG] Industry select dropdown not found."); return; }
        ...
        const init = () => {
            ...
            // *** MODIFIED: Use setTimeout to delay the first analysis run ***
            setTimeout(() => {
                console.log("[DEBUG] Running initial analysis after delay...");
                runAnalysis(); // Run full analysis *after* main.js should be ready
                
                // Apply translations *after* initial run
                if (typeof window.applyTranslations === 'function') {
                    console.log("[DEBUG] Applying translations during init (after delay)...");
                    window.applyTranslations();
                } else {
                    console.error("[!!! DEBUG !!!] applyTranslations function is NOT DEFINED. Check main.js and script order.");
                }
                ...
                if (UI.industrySelect) {
                    UI.industrySelect.addEventListener('change', (e) => {
                        ...
                        // Re-render only the ratio tables
                        renderRatioCategory('liquidityRatios', 'liquidityRatios', ['currentRatio', 'quickRatio']);
                        renderRatioCategory('profitabilityRatios', 'profitabilityRatios', ['grossProfitMargin', 'netProfitMargin', 'roa', 'roe']);
                        renderRatioCategory('leverageRatios', 'leverageRatios', ['debtToAssets', 'debtToEquity']);
                        renderRatioCategory('efficiencyRatios', 'efficiencyRatios', ['assetTurnover']);
                ...
                tabs.forEach(tabId => {
                    ...
                    if (tabElement) {
                        tabElement.addEventListener('shown.bs.tab', () => {
                             ...
                             if (tabId === 'breakeven' && state.breakevenChart) { state.breakevenChart.resize(); } // Resize chart on tab show
                        });
                    } else { console.warn(`[DEBUG] Tab button not found for ID: ${tabId}-tab`); }
                });
                
                console.log("[DEBUG] Advanced page initialization finished.");
    ...
    }, 0); // *** End of main setTimeout wrapper ***
});
