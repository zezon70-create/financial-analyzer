...
document.addEventListener("DOMContentLoaded",(()=>{
    ...
    y=()=>{if(!confirm(o("confirmClearUpload")))return;const e={};l.columnMapper.querySelectorAll("select[data-field-key]").forEach((t=>{e[t.dataset.fieldKey]=t.value}));const s=document.getElementById("map-StatementType").value;if(t.data[s]=[],t.fileData.forEach((a=>{const n=a[e.Classification]||"",l=C(n),i={Account:a[e.Account]||"",Value:i(a[e.Value]),Classification:l},d={};r.tables[s].fields.forEach((e=>d[e]=i[e])),t.data[s].push(d)})),0===t.data[s].length){const e=e=>e.reduce(((e,t)=>({...e,[t]:"Value"===t?0:""})),{});t.data[s]=[e(r.tables[s].fields)]}d(),m();new bootstrap.Tab(l.manualTab).show();const n=document.getElementById(`${s}-tab`);if(n){new bootstrap.Tab(n).show()}g(),alert(o("fileProcessingSuccess"))},
