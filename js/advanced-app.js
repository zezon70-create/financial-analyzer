    ...
    const init = () => {
        ...
        if (UI.industrySelect) {
            UI.industrySelect.addEventListener('change', (e) => {
                ...
                if (typeof applyTranslations === 'function') { applyTranslations(); } // Re-apply translations if needed for dynamic text in table headers etc.
            });
        }
        ...
        tabs.forEach(tabId => {
         ...
         if (state.hasValidData) {
            ...
            // Trigger display functions anyway to show warnings if data is invalid
            calculateAndDisplayDupont(); calculateAndDisplayVerticalAnalysis(); calculateAndDisplayZScore(); calculateAndDisplayCashFlowAnalysis();
         }
