// js/advanced-app.js (Version with enhanced error checking and logging)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = { 
        ...
        ratios: {},
        breakevenChart: null,
        hasValidData: false, // More specific flag
    ...
    const calculateFinancials = () => {
        ...
        state.hasValidData = false; // Reset flag
        let trialData;
        try {
            const rawDataString = localStorage.getItem('trialData');
            if (!rawDataString) {
                console.warn("localStorage 'trialData' is missing.");
                return false;
            }
            ...
            if (!Array.isArray(trialData) || trialData.length === 0 || (trialData.length === 1 && !trialData[0].Account && !toNum(trialData[0].Debit) && !toNum(trialData[0].Credit))) {
                console.warn("Parsed 'trialData' is empty or invalid.");
                return false; 
            }
        } catch (e) {
            console.error("Error parsing 'trialData' from localStorage:", e);
            return false; // Indicate failure due to parsing error
        }

        try { // Add try...catch around the calculation logic
            const f = { /* ... (f object definition as before) ... */
                 ...
                 interestExpense: 0, taxExpense: 0, depreciationAmortization: 0, ppeNet: 0, 
                 longTermDebt: 0, shortTermDebt: 0, cashEquivalents: 0, ebit: 0, workingCapital: 0,
                 // CF Estimates
                 ocf_estimated: 0, capex_estimated: 0, icf_estimated: 0, fcf_estimated: 0, 
                 netCashChange_estimated: 0, freeCashFlow_estimated: 0
             };
            
            trialData.forEach(row => { /* ... (processing logic as before) ... */
            ...
            f.fcf_estimated = 0; // Still assuming 0 as change cannot be determined
            ...
            state.financials = f;
            state.hasValidData = true; // Set flag to true only if calculations succeed
            console.log("Calculated Financials:", f);
            return true; 
        } catch (e) {
            console.error("Error during financial calculations:", e);
            state.financials = {}; // Ensure financials are empty on error
    ...
    const calculateAllRatios = () => {
        state.ratios = {}; // Reset
        if (!state.hasValidData) { // Check the flag first
             console.warn("Financials not calculated or invalid, skipping ratio calculation.");
             return false; 
        }
        ...
        try { // Add try...catch for safety
            ...
            state.ratios = {};
            state.hasValidData = false; // Mark data as invalid if ratio calculation fails
            return false;
        }
    };
    ...
    const renderRatioCategory = (divId, categoryTitleKey, ratioKeys) => { 
        ...
        /* ... (rest of the rendering logic as before) ... */ let tableHTML = `<h5 class="mb-3">${t_page(categoryTitleKey)}</h5> <div class="table-responsive"> <table class="table table-sm table-striped"> <thead><tr><th>${t_page('thRatio')}</th><th>${t_page('thValue')}</th><th>${t_page('thComment')}</th></tr></thead> <tbody>`; ratioKeys.forEach(key => { const value = state.ratios[key]; const isPercentage = key.includes('Margin') || key.includes('roa') || key.includes('roe'); const formattedValue = isPercentage ? formatPercent(value) : formatRatio(value); const comment = getRatioComment(key, value); tableHTML += `<tr> <td>${t_page(key)}</td> <td><strong>${formattedValue}</strong></td> <td class="text-muted small">${comment}</td> </tr>`; }); container.innerHTML = tableHTML + `</tbody></table></div>`; 
    };
    const renderSidebar = () => { 
        // Use state.hasValidData flag 
        if (!state.hasValidData) { /* ... (show no data messages) ... */ UI.smartSummary.textContent = lang === 'ar' ? 'يرجى إدخال بيانات لحساب الملخص.' : 'Please enter data to calculate summary.'; UI.alertsArea.innerHTML = `<div>${lang === 'ar' ? 'لا توجد بيانات كافية للتنبيهات.' : 'Insufficient data for alerts.'}</div>`; return; } 
        /* ... (rest of the rendering logic as before) ... */ const { netProfitMargin, currentRatio, debtToEquity } = state.ratios; UI.smartSummary.textContent = netProfitMargin > 0 && currentRatio > 1.5 ? t_page('summary_ok') : t_page('summary_risk'); const alerts = []; if (currentRatio < 1 && isFinite(currentRatio)) alerts.push(t_page('alert_liquidity_risk')); if (debtToEquity > 2 && isFinite(debtToEquity)) alerts.push(t_page('alert_leverage_risk')); if (netProfitMargin < 0 && isFinite(netProfitMargin)) alerts.push(t_page('alert_profit_risk')); UI.alertsArea.innerHTML = alerts.length > 0 ? alerts.map(alert => `<div>${alert}</div>`).join('') : `<div>${t_page('alert_ok')}</div>`; 
    };
    ...
    const calculateAndDisplayCashFlowAnalysis = () => {
        ...
        if(UI.cfInterpretation) UI.cfInterpretation.innerHTML = interpretation || `<p class="text-muted">${lang==='ar'?'لا يوجد تفسير...':'No interpretation...'}</p>`;
    };
    ...
    const runAnalysis = () => {
        ...
        if (!calculateFinancials()) {
             ...
             state.hasValidData = calculateAllRatios(); // calculateAllRatios returns true/false
        }

        // Always try to render, the render functions will show warnings if !state.hasValidData
        ...
        // Return success state for other functions
        ...
        tabs.forEach(tabId => {
            ...
            if (tabElement) {
                tabElement.addEventListener('shown.bs.tab', () => {
                     ...
                     if (!state.hasValidData) {
                          console.log("No valid data, attempting to re-run analysis...");
                          runAnalysis(); // Try to re-calculate if data was missing
                     }
                     
                     // Trigger display functions for specific tabs
         ...
         // Initial calculations/display for all tabs after first analysis run
