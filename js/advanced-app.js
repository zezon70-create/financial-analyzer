...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        const state = { 
            financials: {}, 
            financialsPrevious: {}, // For Horizontal & CCC Averages
            ratios: {},
            breakevenChart: null,
            baseBreakeven: { units: NaN, value: NaN }, // For Scenario Analysis
            hasValidData: false, 
            hasPreviousData: false, // Flag for Horizontal & CCC Averages
            rawData: { bsItems: [], isItems: [] }, // Current period raw items
            rawDataPrevious: { bsItems: [], isItems: [] } // Previous period raw items
        };
        ...
        const formatChangePercent = (newValue, baseValue) => {
            ...
            const sign = change > 0 ? '+' : ''; // Add '+' for positive changes
        ...
        const calculateAllRatios = () => {
             ...
             try { 
                 ...
                 const costOfSalesForDPO = purchases > 0 ? purchases : cogs; // Use COGS if purchases is 0
        ...
        const calculateAndDisplayHorizontal = () => {
             ...
             if (!state.hasValidData || !state.hasPreviousData) {
              ...
              const generateHorizontalRows = (itemsCurrent, itemsPrev, category) => {
                  ...
                  // Populate map correctly with calculated balance values for previous period
                  itemsPrev.forEach(item => prevItemsMap.set(item.account, item.value));

                  itemsCurrent.forEach(itemCurr => {
                      const currDisplayValue = itemCurr.value; // Use calculated balance for current display
                      const prevDisplayValue = prevItemsMap.get(itemCurr.account) || 0; // Use calculated balance for previous display

                      // Use rawValue for calculating change percentage based on debit/credit flow
                      const currRawValue = itemCurr.rawValue;
                      let prevRawValue = 0;
                      const prevRawItem = itemsPrev.find(item => item.account === itemCurr.account);
                      if (prevRawItem) {
                          prevRawValue = prevRawItem.rawValue;
                      }

                      const changeAbsDisplay = currDisplayValue - prevDisplayValue; // Change in the displayed balance
                      const changePctRaw = (prevRawValue !== 0) ? ((currRawValue - prevRawValue) / Math.abs(prevRawValue)) : (currRawValue !== 0 ? Infinity : 0); // Percent change based on raw flow
                      ...
                      rowsHTML += `<tr>
                          ...
                          <td class="text-end ${changeAbsDisplay > 0 ? 'text-success' : (changeAbsDisplay < 0 ? 'text-danger' : '')}">${formatPercent(changePctRaw)}</td>
                      </tr>`;
                  });
                  return rowsHTML;
              };
             ...
             // Use the stored rawData and rawDataPrevious which now contain 'rawValue' and 'value'
              ...
              if (isNaN(state.baseBreakeven.units) || isNaN(state.baseBreakeven.value)) {
                  if (isFinite(baseFixed) && isFinite(baseVariable) && isFinite(basePrice) && basePrice > baseVariable) {
                     ...
                     console.log("[DEBUG] Base breakeven calculated for scenario:", state.baseBreakeven)
                      ...
                      // Attempt to calculate BE first if values might be present but not calculated yet
                      const calculatedBase = calculateAndDisplayBreakeven(); // Try calculating
                      if (isNaN(calculatedBase?.units)) { // Check if calculation failed
        ...
        const runAnalysis = () => {
            ...
            if (state.hasValidData) { 
                calculateAllRatios(); // Ratios now use averages if previous data exists
            } else {
                state.ratios = {}; 
            }

            // Render ratios based on potentially averaged calculations
        ...
        const init = () => {
            ...
            setTimeout(() => {
                ...
                // Initial display calls for existing tabs
                ...
                // Initial display calls for new tabs (will show warnings if needed)
            ...
            // Tab Change Listeners - Refresh tab content on view
            ...
            tabs.forEach(tabId => {
                ...
                if (tabElement) {
                    tabElement.addEventListener('shown.bs.tab', () => {
                        ...
                        // Re-check data validity specific to the tab needs before re-rendering
                        let needsRefresh = !state.hasValidData;
                        if (tabId === 'horizontal' && !state.hasPreviousData) needsRefresh = true;
                        // Ratios/CCC might need update if previous data loaded after initial run
                        if ((tabId === 'ratios' || tabId === 'ccc') && !state.hasPreviousData) { 
                           // Check if previous data *became* available since load
                        ...
                        if (needsRefresh) { 
                            ...
                            runAnalysis(); // Re-run analysis if data seems missing or might have changed
                        }
                        
                        // Re-render the specific tab's content
                        if (tabId === 'ratios') renderRatioCategory('liquidityRatios', 'liquidityRatios', ['currentRatio', 'quickRatio', 'netWorkingCapital', 'cashRatio']); // Just re-render ratios part
